...
const defaultSettings = {
  ...
  dailyLossLimit: 500,
  enableMaxDrawdownTracker: false,
  drawdownStartingBalance: 50000,
  maxDrawdownAmount: 2000,
  drawdownTargetValue: 50000,
...
function maxDrawdownTrackerData() {
  const startingBalance = Number(state.settings.drawdownStartingBalance) || defaultSettings.drawdownStartingBalance;
  const maxDrawdownAmount = Number(state.settings.maxDrawdownAmount) || defaultSettings.maxDrawdownAmount;
  const targetValue = Number(state.settings.drawdownTargetValue ?? state.settings.drawdownLockPrice) || defaultSettings.drawdownTargetValue;
  const trades = scoringTrades(state.trades).sort(byDate);
  let balance = startingBalance;
  let highWatermark = startingBalance;
  trades.forEach((trade) => {
    balance += Number(trade.net || 0);
    highWatermark = Math.max(highWatermark, balance);
  });
  const startingFloor = startingBalance - maxDrawdownAmount;
  const trailingFloor = highWatermark - maxDrawdownAmount;
  const unclampedFloor = Math.max(startingFloor, trailingFloor);
  const drawdownFloor = Math.min(unclampedFloor, targetValue);
  const locked = unclampedFloor >= targetValue;
  const buffer = balance - drawdownFloor;
  const meterTop = Math.max(targetValue, startingBalance, balance);
  const meterSpan = Math.max(1, meterTop - drawdownFloor);
  const balancePct = clamp((balance - drawdownFloor) / meterSpan * 100, 0, 100);
  return { startingBalance, maxDrawdownAmount, targetValue, balance, highWatermark, drawdownFloor, locked, buffer, balancePct };
}

function renderMaxDrawdownTracker() {
  const panel = $("#maxDrawdownTrackerPanel");
  panel.classList.toggle("hidden", !state.settings.enableMaxDrawdownTracker);
  if (!state.settings.enableMaxDrawdownTracker) return;
  const data = maxDrawdownTrackerData();
  const status = data.locked ? "Locked" : "Not Locked";
  const bufferPct = data.maxDrawdownAmount ? clamp(data.buffer / data.maxDrawdownAmount * 100, 0, 100) : 0;
  const statusClass = bufferPct <= 25 ? "danger" : bufferPct <= 50 ? "warn" : "safe";
  $("#drawdownCurrentBalance").textContent = money(data.balance);
  $("#drawdownFloor").textContent = money(data.drawdownFloor);
  $("#drawdownTargetValue").textContent = money(data.targetValue);
  $("#drawdownStatus").textContent = `Lock Status: ${status}`;
  $("#drawdownStatus").className = `drawdown-status ${data.locked ? "locked" : ""}`;
  $("#drawdownPointer").style.left = `${data.balancePct}%`;
  $("#drawdownPointer").className = `risk-pointer ${statusClass}`;
}
...
function renderReports() {
  ...
  $("#emotionAverageDetail").textContent = trades.length ? "Overall emotion control while trading." : "No trades in this period.";
...
function renderHeaderLevelBadges(view = activeView()) {
  const shouldShow = view !== "milestones";
...
function renderSettings() {
  ...
  $("#settingsForm [name=drawdownStartingBalance]").value = state.settings.drawdownStartingBalance;
  $("#settingsForm [name=maxDrawdownAmount]").value = state.settings.maxDrawdownAmount;
  $("#settingsForm [name=drawdownTargetValue]").value = state.settings.drawdownTargetValue ?? state.settings.drawdownLockPrice ?? defaultSettings.drawdownTargetValue;
  $("#settingsForm [name=enableMaxDrawdownTracker]").checked = Boolean(state.settings.enableMaxDrawdownTracker);
...
$("#settingsForm").addEventListener("submit", (event) => {
  ...
  const drawdownStartingBalance = Number(form.get("drawdownStartingBalance"));
  const maxDrawdownAmount = Number(form.get("maxDrawdownAmount"));
  const drawdownTargetValue = Number(form.get("drawdownTargetValue"));
  state.settings = {
    ...state.settings,
    ...
    enableMaxDrawdownTracker: form.get("enableMaxDrawdownTracker") === "on",
    drawdownStartingBalance: Number.isFinite(drawdownStartingBalance) ? Math.max(0, drawdownStartingBalance) : defaultSettings.drawdownStartingBalance,
    maxDrawdownAmount: Number.isFinite(maxDrawdownAmount) ? Math.max(0, maxDrawdownAmount) : defaultSettings.maxDrawdownAmount,
    drawdownTargetValue: Number.isFinite(drawdownTargetValue) ? Math.max(0, drawdownTargetValue) : defaultSettings.drawdownTargetValue,
